...
export const FoodOdishaPage: React.FC<FoodOdishaPageProps> = ({ onBackHome, onPlanJourney }) => {
  ...
  return (
    <div className="relative isolate min-h-screen overflow-hidden bg-[#faf8f4] pb-12 text-[#172d49]">
      ...
      {activeDish && <div className="fixed inset-0 z-[70] flex items-end bg-[#102342]/50 p-0 backdrop-blur-sm sm:items-center sm:justify-center sm:p-6" role="dialog" aria-modal="true" aria-label={`${activeDish.name} details`} onClick={() => setActiveDish(null)}>
        <article className="max-h-[92vh] w-full max-w-5xl overflow-y-auto rounded-t-[30px] bg-[#fffdf9] shadow-2xl sm:rounded-[30px]" onClick={(event) => event.stopPropagation()}>
          <div className="grid lg:grid-cols-[42%_58%]"><div className="relative min-h-[280px] lg:min-h-full"><img src={activeDish.image} alt={activeDish.name} className="absolute inset-0 h-full w-full object-cover" /><div className="absolute inset-0 bg-gradient-to-t from-[#102342]/70 to-transparent" /><div className="absolute bottom-6 left-6 right-6 text-white"><p className="text-xs font-bold uppercase tracking-[0.18em] text-[#ffe1b1]"><MapPin className="mr-1 inline h-3.5 w-3.5" />{activeDish.region}</p><h2 className="mt-2 font-serif text-4xl font-bold">{activeDish.name}</h2></div></div><div className="relative p-6 sm:p-8"><button type="button" onClick={() => setActiveDish(null)} className="absolute right-5 top-5 rounded-full border border-[#eadfce] bg-white p-2 text-[#172d49] transition hover:text-[#b84a2d]" aria-label="Close dish story"><X className="h-4 w-4" /></button><div className="pr-10"><span className="inline-flex rounded-full bg-[#f5ecdf] px-3 py-1 text-[10px] font-bold uppercase tracking-[0.13em] text-[#a44729]">{activeDish.district} · {activeDish.dietary}</span><p className="mt-4 text-base leading-relaxed text-[#52606e]">{activeDish.story}</p></div><div className="mt-6 grid gap-4 sm:grid-cols-2"><div className="rounded-2xl bg-[#f8f1e7] p-4"><h3 className="font-serif text-lg font-bold">What’s in it</h3><ul className="mt-2 space-y-1.5 text-sm text-[#586575]">{activeDish.ingredients.map((ingredient) => <li key={ingredient} className="flex gap-2"><Check className="mt-0.5 h-3.5 w-3.5 shrink-0 text-[#b84a2d]" />{ingredient}</li>)}</ul></div><div className="rounded-2xl bg-[#f3f6f0] p-4"><h3 className="font-serif text-lg font-bold">When to try it</h3><p className="mt-2 flex gap-2 text-sm leading-relaxed text-[#586575]"><CalendarDays className="mt-0.5 h-4 w-4 shrink-0 text-[#4d7c0f]" />{activeDish.availability}</p><p className="mt-3 text-sm font-semibold text-[#334d34]">{activeDish.bestFor}</p></div></div><div className="mt-4 rounded-2xl border border-[#e7d6bd] bg-[#fff8ed] p-4"><h3 className="flex items-center gap-2 font-serif text-lg font-bold"><UtensilsCrossed className="h-4 w-4 text-[#b84a2d]" />Local food note</h3><p className="mt-1.5 text-sm leading-relaxed text-[#657080]">{activeDish.localNote}</p></div><div className="mt-3 flex gap-3 rounded-2xl border border-[#d6e1d2] bg-[#f3f8f1] p-4"><ShieldCheck className="mt-0.5 h-5 w-5 shrink-0 text-[#4d7c0f]" /><div><h3 className="font-serif text-lg font-bold text-[#28452b]">Try it well</h3><p className="mt-1 text-sm leading-relaxed text-[#4f6352]">{activeDish.safetyNote}</p></div></div><button type="button" onClick={() => { setActiveDish(null); onPlanJourney(activeDish.district); }} className="mt-6 inline-flex w-full items-center justify-center gap-2 rounded-xl bg-[#172d49] px-5 py-3 text-sm font-bold text-white transition hover:bg-[#b84a2d]"><Sparkles className="h-4 w-4 text-[#f6c77b]" />Add this food stop to my journey <ArrowRight className="h-4 w-4" /></button></div></div>
        </article>
      </div>}
    </div>
  );
